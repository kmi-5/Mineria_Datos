...
def exportar_tabla_a_json(df, nombre_tabla):
    """Exporta una tabla individual a formato JSON"""
    try:
        # Crear carpeta JSON si no existe
        carpeta_json = os.path.join(BASE_AUXILIAR, "json_exports")
        os.makedirs(carpeta_json, exist_ok=True)
        
        # Ruta del archivo JSON
        ruta_json = os.path.join(carpeta_json, f"{nombre_tabla}.json")
        
        # Convertir a JSON (formato orient='records' para array de objetos)
...
def menu_interactivo(df_original, nombre):
    ...
    while True:
        ...
        print("6️⃣ Eliminar un registro/fila")
        print("7️⃣ 📤 Exportar tabla a JSON")  # NUEVA OPCIÓN
        print("8️⃣ Finalizar y GUARDAR cambios")
        print("9️⃣ Volver sin guardar cambios")
        print("🔟 Salir del programa")

        opcion = input("Seleccioná una opción (1-10): ").strip()
        ...
        elif opcion == "7":  # NUEVA OPCIÓN - EXPORTAR A JSON
            print("📤 Exportando tabla a formato JSON...")
            if exportar_tabla_a_json(df, nombre):
                cambios.append(f"Exportada tabla a JSON: {nombre}.json")
                print("✅ Exportación completada.")
            else:
                print("❌ No se pudo exportar la tabla.")

        elif opcion == "8":  # finalizar y guardar
            break

        elif opcion == "9":  # volver sin guardar
